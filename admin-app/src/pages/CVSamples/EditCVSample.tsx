...
const EditCVSample: React.FC = () => {
  ...
  const loadCVSample = async () => {
    try {
      ...
      setCvSample(data);
      setDemoImageUrl(data.demoImage || '');
      ...
      message.error('Không thể tải thông tin mẫu CV');
      navigate('/cv-samples');
    } finally {
      setLoading(false);
    }
  ...
  const onFinish = async (values: UpdateCVSamplePayload) => {
    setSaving(true);
    try {
      const cvSampleData = {
        ...values,
      ...
      await updateCVSample(id!, cvSampleData);
      
      message.success('Cập nhật mẫu CV thành công!');
      navigate('/cv-samples');
    } catch (error: any) {
      message.error(error?.response?.data?.message || 'Cập nhật mẫu CV thất bại!');
    } finally {
      setSaving(false);
    }
  };
  ...
  if (loading) {
    return <FormSkeleton fieldCount={15} />;
  }
        ...
        <Row justify="space-between" align="middle">
          <Space>
            ...
            <Title level={3} style={{ margin: 0 }}>
              Chỉnh sửa mẫu CV: {cvSample?.name}
            </Title>
          </Space>
          <Space>
            <Button
